...
void solve(){
    ...
    // 4方向に拡大するときは、T0でかい方が良いかも
    // double T0=200000000.0*n;
    // double T1=200.0*n;
    double T0=2e8 *n;
    double T1=5e3 *n;
    double TL = 4900.0; //Atcoderに投げるときはもうちょい伸ばして良い？
    TL*=2;
    ...
    while(tm.check(TL)){
        ...
        //// transformを完成させた今、 もうこれはない方が精度高そう？
        // if(t01<0.8 && state.ads[adi].pena<0.2*PENA_K)continue;
        ...
        if(rand_01(mt64)<0.0001*t01*t01){
            int slide_dir;
            // if(dir<=1) slide_dir=2+rand_dir4(mt64)%2;
            // else slide_dir=rand_dir4(mt64)%2;
            slide_dir=rand_dir4(mt64);
            slide_ad_max(state, adi, slide_dir);
        }

        ///// single dir
        //  ll dist = rand_hw(mt64)%100 + 1;
        // bool ret = try_to_expand(state, adi, dir, rand_01(mt64), tt, 0, dist);
        ...
        for(int i=0;i<4;i++){
            ...
            if(t01>0.7)dists[i]=rand_hw(mt64)%1600-400; dists[i]=max(dists[i],0);
            if(rand_01(mt64)<0.35*t01) dists[i]*=(-1); //0.3がbest?
        }
        // まだペナがでかい広告は、無理やりでも拡大するために 焼き鈍し確率をあげる
        if(0.5<t01 && t01<0.8 && calc_ad_pena(state.ads[adi])>0.4*PENA_K ){
        ...
        // たまに変形させてみる。縮小&拡大ができた今、要る？
        // if(t01>0.0 && rand_01(mt64)<0.01){
        //     try_to_transform(state, adi);
        // }

        ////たまに消してみる？
        // if(t01<0.75 && rand_01(mt64)<0.0000001){
        //     Ad &ad=state.ads[adi];
        ...
        for(int adi=0 ; adi<n ; adi++){
            while(tm.check(TL)){
                ...
                for(int dir=0; dir<4 ; dir++){
                    ...
                    bool ret;
                    // bool ret = try_to_expand(state,adi,dir,1.1,(1.0)*PENA_K, 0, dist);
