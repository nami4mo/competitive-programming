...
fn vis(input: &Input, out: &Vec<Rect>, show_id: bool) {
	...
	if show_id {
		...
		for i in 0..input.ps.len() {
			let val = if out[i].size() > input.size[i] {
				1.0 - input.size[i] as f64 / out[i].size() as f64
			} else {
				out[i].size() as f64 / input.size[i] as f64
			};
			let cx = (out[i].x1 + out[i].x2) as f64 / 2.0;
			let cy = (out[i].y1 + out[i].y2) as f64 / 2.0;
			let val2 = (val*100.0) as i64;
			doc = doc.add(svg::node::element::Text::new().set("x", cx).set("y", cy + fontsize * 0.35).set("font-size", fontsize).set("text-anchor", "middle").add(svg::node::Text::new(format!("{}", (val2)))));
		}
	}
