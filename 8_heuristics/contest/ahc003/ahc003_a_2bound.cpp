...
uniform_int_distribution<int> rand_lcr(0, 2);
...
struct StreetInfo{
    ...
    ll est_dist_c = default_dist; // 最初は0にしておくことで、優先的に使われる？
    ...
    void add_est_dist(ll dist, int lr){
        ...
        if(lr==0) {
            ll next_dist = est_dist_c + dist;
            est_dist_c=max(min_dist, next_dist);
            est_dist_c=min(max_dist, est_dist_c);
        }
        if(lr==2) {
    ...
    ll get_dist(int pos){
        if(pos<bound1) return est_dist_l;
        if(pos<bound2) return est_dist_c;
        else return est_dist_r;
    }
    int check_lr(int pos){
        if(pos<bound1) return 0;
        if(pos<bound2) return 1;
        else return 2;
    }
...
void update_gl_from_est_dist(State &state, int col_row, int ind, int lr){
    if(col_row==0){
        ...
        int bound1 = street.bound1;
        int bound2 = street.bound2;
        if(lr==0){
            ...
            for(int y=bound1 ; y<bound2 ; y++){
                int pos = get_pos_num(y,x);
                state.grid_gl[pos].edges[0]={get_pos_num(y-1,x), street.est_dist_c};
                state.grid_gl[pos].edges[1]={get_pos_num(y+1,x), street.est_dist_c};
            }
        }
        else{
            for(int y=bound2 ; y<HW ; y++){
                int pos = get_pos_num(y,x);
        ...
        if(lr==0){
            ...
            for(int x=bound1 ; x<bound2 ; x++){
                int pos = get_pos_num(y,x);
                state.grid_gl[pos].edges[2]={get_pos_num(y,x-1), street.est_dist_c};
                state.grid_gl[pos].edges[3]={get_pos_num(y,x+1), street.est_dist_c};
            }
        }
        else{
            for(int x=bound2 ; x<HW ; x++){
                int pos = get_pos_num(y,x);
...
void update_route2(State &state, double t01){
    ...
    // if(kaizen>0){
        for(EdgeUpdateInfo &eui : edge_cands){
            if(eui.col_row==0)  state.col_streets[eui.ind].add_est_dist(eui.dist_d, eui.lr);
            else                state.row_streets[eui.ind].add_est_dist(eui.dist_d, eui.lr);
...
void update_streets_from_moveres_2(State &state, MoveRes move_res, bool update_all=false){
    ...
    vector<int> row_move_cnts_c(HW,0);
    vector<int> col_move_cnts_c(HW,0);
    ...
    for(char dir : move_res.route){
        if(dir == 'U' || dir == 'D') {
            ...
            if(lr==1 || update_all) col_move_cnts_c[px]+=1;
            if(lr==2 || update_all) col_move_cnts_r[px]+=1;
        }
        else{
            int lr = state.row_streets[py].check_lr(px);
            if(lr==0 || update_all) row_move_cnts_l[py]+=1;
            if(lr==1 || update_all) row_move_cnts_c[py]+=1;
            if(lr==2 || update_all) row_move_cnts_r[py]+=1;
        }
        if(dir=='U') py--;
        else if(dir=='D') py++;
        else if(dir=='L') px--;
    ...
    for(int y=0 ; y<HW ; y++){
        ...
        if(row_move_cnts_l[y] > 0){
            ...
            street.add_est_dist(update_dist*contribute, 0);
        }
        if(row_move_cnts_c[y] > 0){
            double contribute = (1.0) * row_move_cnts_c[y] / step;
            street.est_dist_c = street.est_dist_c+update_dist*contribute;
            street.add_est_dist(update_dist*contribute, 1);
        }
        if(row_move_cnts_r[y] > 0){
            ...
            street.add_est_dist(update_dist*contribute, 2);
        }
        int bound1 = street.bound1;
        int bound2 = street.bound2;
        if(row_move_cnts_l[y] > 0 ){
    ...
    for(int x=0 ; x<HW ; x++){
        ...
        if(col_move_cnts_c[x] > 0){
            double contribute = (1.0) * col_move_cnts_c[x] / step;
            street.est_dist_c = street.est_dist_c+update_dist*contribute;
            street.add_est_dist(update_dist*contribute, 1);
        }
...
void solve(){
    ...
    REP(i,QUERY_N){
        ...
        bool update_all_flag = true;
        if(i<1000) update_all_flag = false;
        update_streets_from_moveres_2(state, res, update_all_flag);
